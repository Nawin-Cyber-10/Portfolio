    ...
    <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
      {leadershipRoles.map((role, index) => (
        <motion.div
          ...
          viewport={{ once: true }}
        >
          <Card className="bg-gray-800/80 backdrop-blur-sm border-gray-700 h-full hover:border-purple-500/50 transition-all duration-300 overflow-hidden group purple-glow-hover">
            <div className="absolute top-0 left-0 w-full h-1 bg-gradient-to-r from-purple-500 to-violet-500 transform origin-left scale-x-0 group-hover:scale-x-100 transition-transform duration-300"></div>
            <CardHeader className="flex flex-row items-center gap-4 pb-2">
              <div className="bg-gray-900/80 p-3 rounded-lg">{role.icon}</div>
              <div>
                <CardTitle className="text-xl">{role.title}</CardTitle>
                <CardDescription className="text-gray-400">{role.organization}</CardDescription>
              </div>
            </CardHeader>
            <CardContent>
